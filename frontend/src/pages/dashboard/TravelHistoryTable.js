...
function TravelHistoryTable() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        const response = await fetch('http://localhost:5000/api/travelhistory/');
        if (!response.ok) {
  ...
  const handleDelete = async (recordId) => {
    try {
      const response = await fetch(`http://localhost:5000/api/travelhistory/${recordId}`, {
        method: 'DELETE',
      });
            ...
            <TableBody>
              {filteredTravelHistory.slice(startIndex, endIndex).map((record) => (
                <TableRow key={record._id}>
                  ...
                  <TableCell>INR {record.cost}</TableCell>
                  <TableCell>
                    <Button
                      variant="outlined"
                      color="error"

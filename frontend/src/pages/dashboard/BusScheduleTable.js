...
const BusScheduleTable = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch('http://localhost:5000/api/bus-schedules');
        if (!response.ok) {
  ...
  const handleSaveEdit = async () => {
    try {
      const response = await fetch(`http://localhost:5000/api/bus-schedules/${editingData._id}`, {
        method: 'PUT',
        headers: {
  ...
  const confirmDelete = async () => {
    try {
      const response = await fetch(`http://localhost:5000/api/bus-schedules/${itemToDelete._id}`, {
        method: 'DELETE',
      });

...
function PassengerTable() {
  ...
  const handleTopUp = async () => {
    try {
      ...
      const response = await fetch(`http://localhost:5000/api/localpassengers/${topUpPassengerId}`, {
        method: 'PUT',
        headers: {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        const response = await fetch('http://localhost:5000/api/localpassengers');
        if (!response.ok) {
  ...
  const handleDelete = async (passengerId) => {
    try {
      const response = await fetch(`http://localhost:5000/api/localpassengers/${passengerId}`, {
        method: 'DELETE',
      });
      if (!response.ok) {
          ...
          <Table>
            <TableHead>
              <TableRow>
                ...
                <TableCell>Balance (INR)</TableCell>
  ...
  {passenger.password.slice(0, -1).replace(/./g, '*') + passenger.password.slice(-1)}
      ...
      >
        ...
        <DialogContent>
          <DialogContentText>
            Last TopUp Amount: <Chip label={`INR ${lastTopUpAmount}`} />
          </DialogContentText>

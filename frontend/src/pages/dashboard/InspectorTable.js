...
function InspectorTable() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        const response = await fetch('http://localhost:5000/api/busInspectors/');
        if (!response.ok) {
    ...
    async function fetchRoutes() {
      try {
        const response = await fetch('http://localhost:5000/api/busroutes/');
        if (!response.ok) {
  ...
  const handleDelete = async (inspectorId) => {
    try {
      const response = await fetch(`http://localhost:5000/api/busInspectors/${inspectorId}`, {
        method: 'DELETE',
      });
      if (!response.ok) {
  ...
  const handleSaveEdit = async (rowId) => {
    try {
      ...
      const response = await fetch(`http://localhost:5000/api/busInspectors/${rowId}`, {
        method: 'PUT',
        headers: {

...
const FreeRevenuePage = () => {
    ...
    useEffect(() => {
        ...
        fetch('http://localhost:5000/api/travelhistory/')
    ...
    useEffect(() => {
        // Fetch data from the API endpoint
        fetch('http://localhost:5000/api/localpassengers/')
            .then((response) => response.json())
            .then((data) => {
    ...
    useEffect(() => {
        // Fetch data from the API endpoint
        fetch('http://localhost:5000/api/businspectors/')
            .then((response) => response.json())
            .then((data) => {
                // Assuming the response data is an array of documents, and you want to count them
    ...
    return (
        <Grid container rowSpacing={4.5} columnSpacing={2.75}>
            ...
            <Grid item xs={12} sm={6} md={4} lg={3}>
                <AnalyticEcommerce title="Total Revenue" count={costData !== null ? `INR ${costData.toFixed(2)}` : 'Loading...'} />
            </Grid>
            <Grid item xs={12} sm={6} md={4} lg={3}>
                <AnalyticEcommerce title="Total Account Balance of Passengers" count={totalBalance !== null ? `INR ${totalBalance.toFixed(2)}` : 'Loading...'} />
            </Grid>

            <Grid item xs={12} sm={6} md={4} lg={3}>
                <AnalyticEcommerce title="Total Top-Up Amount of Passengers" count={totalTopUpAmount !== null ? `INR ${totalTopUpAmount.toFixed(2)}` : 'Loading...'} />
            </Grid>
            <Grid item xs={12} sm={6} md={4} lg={3}>
